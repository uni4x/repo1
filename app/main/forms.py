...
class CustomerInfoForm(FlaskForm):
    ...
    number_of_guests = IntegerField('ご宿泊のご予定人数', validators=[
        DataRequired(message="予定人数は必須項目です。"),
        NumberRange(min=1, message="予定人数は1人以上である必要があります。")
    ])
    note = StringField('備考')
    payment_type = SelectField('お支払い方法', choices=[('credit_card', 'クレジットカード'), ('cash', '現金'), ('online', 'オンライン')], validators=[DataRequired(message="支払い方法は必須項目です。")])
