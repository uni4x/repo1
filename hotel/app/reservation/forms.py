...
class ReservationForm(FlaskForm):
    checkin_date = DateField('チェックイン', format='%Y-%m-%d', validators=[DataRequired()])
    checkout_date = DateField('チェックアウト', format='%Y-%m-%d', validators=[DataRequired()])
    ...
    payment_type = SelectField('支払い方法', choices=[('credit_card', 'クレジットカード'), ('cash', '現金'), ('online', 'オンライン')], validators=[DataRequired()])
    payment_status = SelectField('支払い状況', choices=[('paid', '支払済み'), ('pending', '未払い'), ('failed', '支払い失敗')], validators=[DataRequired()])
    number_of_guests = IntegerField('人数', validators=[DataRequired(), NumberRange(min=1)])
    status = SelectField('予約状況', choices=[('confirmed', '予約確定'), ('checked_in', 'チェックイン'), ('checked_out', 'チェックアウト')], validators=[DataRequired()])

    def __init__(self, *args, **kwargs):
        super(ReservationForm, self).__init__(*args, **kwargs)
        self.customer_id.choices = [(c.id, c.name) for c in Customer.query.order_by(Customer.name).all()]
    ...
    def update_room_choices(self, checkin_date=None, checkout_date=None):
        ...
        reserved_room_ids = db.session.query(Reservation.room_id).filter(
